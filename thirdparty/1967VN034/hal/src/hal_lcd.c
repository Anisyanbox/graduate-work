...
void HAL_LCD_Setup( LCD_Conf_type *pxCfg )
{
	...
	LX_LCD->VSIZE.field.VSIZE = pxCfg->usHSize * pxCfg->usVSize / 									// ������ ������ ������������ � 32-������ ������
	...
	wa_delay();
	LX_LCD->HTIM.field.HPL = ~pxCfg->bHSyncActiveLevel;												// �������� �������: 0 - �������, 1 - ������
	wa_delay();
	...
	wa_delay();
	LX_LCD->VTIM.field.VPL = ~pxCfg->bVSyncActiveLevel;												// �������� �������: 0 - �������, 1 - ������
	wa_delay();
	...
	LX_LCD->CTRL.field.VBGR = pxCfg->bSwapRgbToBgr;													// ������� ���������� ������ BGR ��� RGB
	wa_delay();
	LX_LCD->CTRL.field.CD = ( pxCfg->ulRgbMode >> 2 ) & 0x3;										// 0 - 8bit, 1 - 16bit, 2 - 24bit, 3 - 32bit � �������
	wa_delay();
	LX_LCD->CTRL.field.VBL = ( pxCfg->ulRgbMode >> 0 ) & 0x3;										// ��������� ��� � �������
	wa_delay();
	LX_LCD->CTRL.field.BL = ~pxCfg->bDReadyActiveLevel;												// �������� ������� ����� ���������� ������ (0 - DRDY �������� �������, 1 - DRDY �������� ������)
	wa_delay();
	LX_LCD->CTRL.field.W2W_EN = ( pxCfg->pxWindow != 0 );											// ���� 1, �� ������������ �������� HDxTIM � VDxTIM
	wa_delay();
	LX_LCD->CTRL.field.PXP_EN = pxCfg->bStopPclkWhenNoData;											// ���� 1, �������� ������� ������������ � ������ ���� � �������� FIFO ��� ������
}

void HAL_LCD_PwmSetDuty( uint8_t ucValue )
{
	if ( ucValue >= HAL_LCD_MAX_PWM_DUTY ) 															// ���������� ��������� �������� ��� ��������� duty == reload
					...
					void *pvDmaIsrHandler )
{
	...
	*( ptr + 3 ) |= ( ( uint32_t ) pvLcdBuf < 0x0C000000 ) ? TCB_INTMEM : TCB_EXTMEM;				// �������� � �������/���������� ������
	...
	HAL_DMA_WriteDC( ulChannel, &tcb );																// ������ DMA
}
