...
void HAL_SPI_Init (SPI_TypeDefInit *SPI) {
	...
	// 2. Инициализация регистра SPCR0:
	...
	// 3. Инициализация регистра SPCR1:
...
void HAL_SPI_DefaultInitStruct (SPI_TypeDefInit *SPI) {
	...
	SPI->CLK_Phase 		= SPI_CLK_Phase_Negedge;			// активность по спадающему фронту
	SPI->CLK_Polarity 	= SPI_CLK_Polarity_High;			// выскокий уровень CLK
	SPI->CLK_Prescaler 	= 2;								// предделитель частоты на (2 + 1)
	SPI->CSNum			= 0;								// обмен по сигналу СS0
	SPI->CS_Active 		= SPI_CS_Active_Low;				// активный уровень CS - низкий
	SPI->FirstBit 		= SPI_FirstBit_MSB;					// первым выдает старший байт слова
	SPI->Mode 			= SPI_Mode_Master;					// режим работы - ведущий
	SPI->CS_Hold 		= SPI_CS_Unhold;					// сигнал CS не удерживается в активном состоянии
	SPI->WordSize 		= 8;								// размер выдаваемого слова - 8 бит
	SPI->LoopBack 		= SPI_LoopBack_Off;					// тествоый режим возврата данных выключен
