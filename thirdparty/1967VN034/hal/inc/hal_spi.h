...
typedef struct {
  __IO SPI_SPCR0  SPCR0;        /*!< Address offset: 0x00 */
  __IO SPI_SPCR1  SPCR1;        /*!< Address offset: 0x01 */
  __IO uint       SPDR;         /*!< Address offset: 0x02 */
  __IO SPI_SPSR   SPSR;         /*!< Address offset: 0x03 */
  __IO uint       RX_CNT;       /*!< Address offset: 0x04 */
} SPI_TypeDef;

#define LX_SPI                 ((SPI_TypeDef *) base_SPI)
...
typedef enum { 
  SPI_FirstBit_MSB	= (0 <<  8),   
	SPI_FirstBit_LSB	= (1 <<  8)
}	SPI_FIRSTBIT;
...
extern "C" {
	...
	SPI_ExchangeMode  HAL_SPI_GetExchangeMode(void);
