...
typedef enum {UART_WordLen_8bit = 0,		UART_WordLen_7bit = (1<<13), 		UART_WordLen_6bit = (2<<13), 		UART_WordLen_5bit = (3<<13)} 			UART_WordLen_type;
typedef enum {UART_TXD_Direct = 0, 			UART_TXD_Low = ((1<<8)|(1<<15)), 	UART_TXD_High = ((1<<8)|(0<<15)),  	UART_TXD_Compliment = ((0<<8)|(1<<15))} UART_TXD_type;
typedef enum {UART_Parity_Off = 0,			UART_Parity_Even = (1<<9),			UART_Parity_Odd = ((1<<9)|(1<<10))} UART_Parity_type;
typedef enum {UART_Mode_TxRx = 0,			UART_Mode_Tx = ((1<<2)|(1<<5)),		UART_Mode_Rx = ((1<<1)|(1<<4))}	 	UART_Mode_type;
typedef enum {UART_Stop_1bit = 0, 			UART_Stop_2bit = (1<<11)} 			UART_StopBits_type;
typedef enum {UART_FIFOSize_Byte = 0, 		UART_FIFOSize_DWord = (1<<12)} 		UART_FIFOSize_type;
typedef enum {UART_DMACtrl_Dis = 0,			UART_DMACtrl_En = 1}				UART_DMACtrl_type;
typedef enum {UART_IT_Dis = 0,				UART_IT_En = 1}						UART_IT_type;
typedef enum {UART_OverSampling_High = 0, 	UART_OverSampling_Normal = (1<<6)}	UART_OverSampling_type;
